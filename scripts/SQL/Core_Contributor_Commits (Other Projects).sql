  ...
  WHILE p_index < p_count DO
    SET p_next = projects[OFFSET(p_index)];
      WHILE t_index < t_count DO
        ...
        CREATE TABLE IF NOT EXISTS `praxis.ccc_commits`
        ...
        OPTIONS(
          ...
          description="Export core contributor commit tasks.  Expires in 2021"
        );
        INSERT INTO `praxis.ccc_commits`
        ...
        INNER JOIN `praxis.core_contributors` as ec
        ...
        (
          ...
          LEFT JOIN `gwu-praxis.praxis.ghtorrent_commits` as c
          ON c.sha = tt2.SHA
          LEFT JOIN `praxis.core_contributors` as ec
          on c.commiter_id = ec.committer_id
          WHERE task != t_next
          AND key = p_next
